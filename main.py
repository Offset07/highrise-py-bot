...
class Bot(BaseBot):
    def __init__(self):
        super().__init__()
        self.bot_id = "01b50864d754b0b8522967751d2b34d252b2e8bcc065ef7327b70dfb9d14aaa6"
        self.owner_id = "daniel_offset"
        self.room_name = "bestieclub"
        self.bot_name = "offsetbot"
    ...
    async def on_user_join(self, user: User) -> None:
        ...
        await self.highrise.chat(f"سلام، خوش امدید به {self.room_name} {user.username} از دنس1 تا دنس55 را بنویسید")
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        ...
        for user, position in room_users.content:
            if user.id == self.bot_id:
                ...
                self.door = position
        try:
            await self.highrise.teleport(self.bot_id, Position(self.door.x, self.door.y, self.door.z-2))
            await self.highrise.walk_to(Position(self.door.x, self.door.y, self.door.z-2, "FrontLeft"))
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.startswith("dance") or message.startswith("دنس"):
        ...
        elif message.strip().lower().startswith("بیا بات") or message.strip().lower().startswith("follow"):
            if user.id == self.owner_id:
                ...
                if privilege_response.moderator or privilege_response.designer:
                    ...
                    print("Follow command only accessible to owners/moderators")
                    return
        elif message.strip().lower() == "اسپم" or message.strip().lower() == "spam":
            if not self.spamming:
                self.spamming = True
                await self.send_spam_messages()
                self.spamming = False
        elif message.strip().lower() == "teleport up" or message.strip().lower() == "تلپورت بالا":
            await self.teleport(user, Position(x=11.5, y=10.25, z=6.5, facing='FrontLeft'))
        elif message.strip().lower() == "teleport down" or message.strip().lower() == "تلپورت پایین":
            await self.teleport(user, Position(x=4.5, y=0.0, z=12.0, facing='FrontLeft'))
        elif message.strip().lower().startswith("teleport to ") or message.strip().lower().startswith("تلپورت به "):
            target_username = message.split("@")[-1].strip()
            await self.teleport_to_user(user, target_username)
        elif any(message.strip().lower().startswith(phrase) for phrase in self.greetings) and self.bot_name.lower() in message.lower():
            await self.reply_to_hello(user)
    ...
    async def send_spam_messages(self) -> None:
        ...
        for _ in range(10):  # Adjust the number of spam messages you want to send
